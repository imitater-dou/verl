...
backend, no source), and `--target=lock` regenerates `uv.lock`. Use
`docker/Dockerfile.uv.cu130` for the cu13.0 / torch-2.11 backends (vllm, sglang,
fsdp, megatron, cpu); the former cu12.9 / torch-2.9.1 image
(`Dockerfile.uv.cu129`, veomni / nemoautomodel) was removed and stays deferred,
as does trtllm. For the full story — the
